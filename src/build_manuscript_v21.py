...
replace_para("Expressed as marginal abatement cost against Type IL mortar",
     f"Expressed as marginal abatement cost against Type IL mortar (Eq. S22; Fig. 8), fly ash and cost-based LC3 abate at −\\${N['mac_fa']} and −\\${N['mac_lc3']} t^−1^ CO~2~e (they save money), BioCarb slurry at −\\${N['mac_bcb']} t^−1^, and the dried route at +\\${N['mac_bca']} t^−1^. Against the next-best SCM mortars the sign changes: BioCarb slurry abates only {N['dg_fa']} and {N['dg_lc3']} kg CO~2~e kg^−1^ more than the fly-ash and LC3 mortars, at a higher price, giving +\\${N['mac_bcb_fa']} and +\\${N['mac_bcb_lc3']} t^−1^, and the dried route does not abate at all relative to either. Fig. 8 compares these values with the only published benchmarks on the same footing---a concrete producer buying cement and SCMs at market prices, derived from the SDSN batching-plant study^49^ (−\\$11 to −\\$39 t^−1^ for slag, fly ash, limestone and calcined-clay substitution)---and with the premium a cement buyer would pay per tonne avoided if kiln CO~2~ capture and storage (\\$50--240 t^−1^)^46--48^ were passed through in the cement price.")
replace_para("Two qualifications govern the reading.",
     f"Three points follow. First, the fly-ash and LC3 savings depend on the price spread between cement and the substitute: the fly-ash mortar saves \\${N['mac_fa']} t^−1^ with fly ash at \\$60 t^−1^ but only \\${N['mac_fa90']} t^−1^ at \\$90 t^−1^ (Monte Carlo range in Fig. 8), and the SDSN values are smaller still because that study priced SCMs within about \\$20 t^−1^ of cement; BioCarb's saving against Type IL rests instead on a feedstock that is available wherever concrete is demolished. Second, as an incremental step beyond fly ash or LC3, BioCarb costs more per tonne avoided than passing kiln CCS through the cement price would, so its niche is displacing plain Type IL where fly ash is scarce, not out-competing LC3. Third, all of these values hold only because clinker is displaced---the 0.010 kg of CO~2~ newly fixed per kg of mortar is worth 0.0075 kg CO~2~e of avoided plant emissions, one eighth of the clinker emissions avoided---and the purchased CO~2~ at \\$60 t^−1^ costs BioCarb {N['co2_cpkg']} ¢ kg^−1^, or \\${N['co2_per_t']} per tonne avoided.")
